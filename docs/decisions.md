...
| 88 | Hammering a create never decays below a floor | Novelty decay says "we have tested this" and hammering a create also means "the app is still too small". Every wave leaves rows behind, and two of the eleven planted bugs cannot exist until hundreds of them do: an unbounded query is fast on an empty table, and a list cannot lose a row it does not have. Decay stalled seasoning at 138 rows in five separate runs; a create keeps a floor of 0.25 so it is scheduled steadily without monopolising |
| 87 | A list is only judged on paging if it actually pages | Shoal is the one adding `?page=2`, and plenty of endpoints accept it and ignore it, returning everything every time. A walk of one of those sees page one again and reports the repeats as lost rows — which is Shoal grading the app on a feature Shoal invented. Page two is compared with page one first; identical means the endpoint does not page and there is nothing to lose |
| 86 | A create is checked against its own receipts, never against a count | Counting cannot be made to work here however carefully it is calibrated, because fifteen other workers are inserting into the same collection throughout — "the total should have moved by eight" is a claim about a number nobody controls, and it reported a plain INSERT endpoint as losing writes in two consecutive runs. Every create hands back the id of the row it made; asking for each of them afterwards is immune to anything else happening on the app, which is the entire point |
